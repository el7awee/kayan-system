...
function initializeSystem() {
  ...
  if (!usersSheet) {
    ...
    usersSheet.getRange(1, 1, 1, 14).setValues([[
      ...
      "Session_Token", "Token_Expiry", "IsDeleted", "Current_Balance", "CSRF_Token"
    ]]);
  }
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (dbUsername === username) {
      if (verifyPassword(password, dbPasswordHash)) {
        ...
        let csrfToken = generateSessionToken(); // CSRF token (random UUID-based)
        ...
        sheet.getRange(i + 1, 14).setValue(csrfToken); // Column N = CSRF_Token
