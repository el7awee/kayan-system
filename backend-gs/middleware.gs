 ...
 * يُستدعى من Code.gs:handleRequest قبل routeRequest
 * يقوم بـ: التحقق من التوكن ← صلاحية الوصول ← CSRF (للعمليات الكتابية)
 * يُرجّع { userId, role } بعد التحقق
 */
function executeMiddlewarePipeline(e, action) {
  let token = e.parameter.Session_Token;
  let authResult = validateAuthenticationAndRBAC(token, action);
  let realUserId = authResult.user_id;
  let realUserRole = authResult.role;
  
  // CSRF check على عمليات الكتابة فقط
  if (checkIfWriteOperation(action)) {
    validateCSRFToken(e, realUserId);
  }
  
  return { userId: realUserId, role: realUserRole };
}

/**
 * 1️⃣ دالة التحقق من الهوية والصلاحيات
 * @returns {Object} authResult من authService_validateToken (يحتوي user_id, role, ...)
 */
function validateAuthenticationAndRBAC(token, action) {
  if (!token || token === "null" || token === "") {
    throwBusinessError("INVALID_TOKEN", "جلسة المستخدم غير صالحة أو انتهت، برجاء تسجيل الدخول مجدداً.");
  }
  
  let authResult = authService_validateToken(token, null);
...
/**
 * 🆕 التحقق من رمز CSRF في عمليات الكتابة
 */
function validateCSRFToken(e, userId) {
  let csrfToken = e.parameter.CSRF_Token;
  if (!csrfToken) {
    throwBusinessError("CSRF_MISSING", "طلب غير مصرح به (نقص رمز الحماية).");
  }
  
  let sheet = getCachedSheet("Users");
  if (!sheet) {
    throwBusinessError("SYSTEM_ERROR", "شيت المستخدمين غير متاح.");
  }
  let data = getCachedData("Users");
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === userId) {
      let storedCsrf = data[i][13] || "";
      if (storedCsrf !== csrfToken) {
        throwBusinessError("CSRF_INVALID", "طلب غير مصرح به (رمز الحماية غير صالح).");
      }
      return;
    }
  }
  throwBusinessError("USER_NOT_FOUND", "المستخدم غير موجود.");
}
